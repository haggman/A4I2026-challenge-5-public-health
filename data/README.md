...
| `care_sites` | one row per facility | every federally recognised care site inside your county, with coordinates, address and `site_kind` |
...
| `shortage_areas` | one row per HRSA designation | where the federal government says a **neighbourhood** is underserved, with `HPSA_SCORE`. Legitimately empty in four of our six counties |
...
`exposure_tracts` carries an eleven-digit tract ID over a 12 km modelling surface.
...
Two traps specific to this domain. **Federal funding does not imply federal licence terms**—the best
